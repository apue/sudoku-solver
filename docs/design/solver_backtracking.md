...
本文件定义 v1 的回溯求解器语义与“多解检测”口径。v1.1 在回溯之前增加了候选集推理（naked single / hidden single），用于减少搜索。实现细节（如 MRV、值排序）不做强制要求，但必须满足下面的可观测行为。
...
- 初始化每个空格的候选集（依据行/列/宫约束）
- 迭代执行：
  - **Naked single**：若某格候选只剩一个值，直接填入
  - **Hidden single**：若行/列/宫内某数字只出现在一个候选格，则填入
- 推理产生的赋值通过事件 `rec.attempt.assign(..., source="deduced")` 记录，Metrics/Stats 也随之增加
- 一旦无法继续推理，则进入回溯阶段；遇到矛盾则直接回滚本轮推理
- CLI 允许通过 `--no-deductions` 禁用该阶段，退回纯回溯
